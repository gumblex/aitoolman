...
## 核心功能
- **统一调用接口**：流式输出或批量调用接口一致；按需构造模板化请求、含上下文的原始请求，简化开发体验。
- **提示词模板化**：将提示词封装为可复用的 Jinja2 模板，集中管理，避免散落各处。
- **灵活的工作流编排**：支持串行、并行及动态工作流，动态决定执行路径，轻松处理复杂多步骤任务。
- **原生工具调用支持**：用经典函数调用模式，或利用工具调用实现意图识别。
- **微服务架构**：可将 LLM 调度器部署为独立服务，实现统一的资源管理、密钥隔离与全局审计。
- **模型路由**：通过标签分组模型，支持多标签匹配、自动权重计算，便于统一调整模型资源，平衡成本和效果。
...
3. [配置](config.md) LLM提供商API密钥和模型参数 (llm_provider.toml)
...
- **输出格式明确**：指定清晰的输出格式（如JSON、XML、Markdown）并给出示例
